    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            icon: const Icon(Icons.history_rounded, color: _textMain),
      ...
      body: StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance
            ...
            .where(_isPsi ? 'psychologistId' : 'patientId', isEqualTo: user.uid)
        ...
        builder: (context, snap) {
          ...
          for (var doc in docs) {
            ...
            if (dt != null &&
                dt.isBefore(now.subtract(const Duration(hours: 1)))) {
              past.add(doc);
            } else {
              upcoming.add(doc);
            }
          ...
          upcoming.sort((a, b) {
            ...
            return (DateTime.tryParse('${ad['date']} ${ad['startTime']}:00') ??
                    DateTime.now())
                .compareTo(
                  DateTime.tryParse('${bd['date']} ${bd['startTime']}:00') ??
                      DateTime.now(),
                );
          });
          ...
          past.sort((a, b) {
            ...
            return (DateTime.tryParse('${bd['date']} ${bd['startTime']}:00') ??
                    DateTime.now())
                .compareTo(
                  DateTime.tryParse('${ad['date']} ${ad['startTime']}:00') ??
                      DateTime.now(),
                );
          });
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        return _isPsi
            ...
            : _PatientAppointmentCard(data: d, isPast: isPast);
      },
    );
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () => Navigator.push(
        ...
        MaterialPageRoute(
          builder: (_) =>
              AppointmentDetailScreen(appointmentData: data, isPast: isPast),
        ),
      ),
      child: Container(
                      ...
                      children: [
                        ...
                        GestureDetector(
                          onTap: () => setState(() {
                            ...
                            if (!_editingLink) {
                              _linkCtrl.text =
                                  widget.data['meetingUrl'] as String? ?? '';
                            }
                          }),
                          child: Container(
                            padding: const EdgeInsets.symmetric(
      ...
      child: ElevatedButton.icon(
        onPressed: () async {
          if (meetingUrl != null && meetingUrl!.isNotEmpty) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('El psicólogo aún no ha asignado un link.'),
              ),
            );
          }
        },
